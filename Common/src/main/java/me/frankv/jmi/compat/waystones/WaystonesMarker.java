...
public enum WaystonesMarker implements ToggleableOverlay {
    ...
    @Getter
    private final Set<WaystoneMeta> waystones = new HashSet<>();
    ...
    private void createMarker(WaystoneMeta waystone) {
        final var marker = new ResourceLocation(Constants.MOD_ID, "images/waystone.png");
    ...
    private void removeMarker(WaystoneMeta waystone) {
        final var marker = markers.remove(waystone);
        if (marker == null) return;
        OverlayHelper.removeOverlay(marker);
    }
    ...
    public void onKnownWaystones(KnownWaystonesEvent event) {
        ...
        final var newWaystones = new HashSet<>(WaystoneMeta.fromEvent(event));
        final var oldWaystones = new HashSet<>(markers.keySet());

        // KnownWaystonesEvent gives a list with only one waystone in it when a new waystone gets placed. That is why this exists.
    ...
    record WaystoneMeta(UUID uuid, String name, BlockPos pos, ResourceKey<Level> dim) {
        public static Set<WaystoneMeta> fromEvent(KnownWaystonesEvent event) {
            final var waystones = new HashSet<WaystoneMeta>();

            event.getWaystones().forEach(w -> {
                if (!w.hasName()) return;
                waystones.add(new WaystoneMeta(w.getWaystoneUid(), w.getName(), w.getPos(), w.getDimension()));
            });
